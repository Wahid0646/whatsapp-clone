...
async function addGroupToSideBar(group){
    ...
    lastMessage.style.color = '#a19d99';
    lastMessage.setAttribute('class', 'text');

    const lastMessageTime = document.createElement('p');
    lastMessageTime.style.color = '#a19d99';
    lastMessageTime.setAttribute('class', 'time text small');
...
async function showGroup(id, name){
    ...
    if(checkGroupAdmin.data){
        ...
        adminBtn.setAttribute('class', 'btn btn-light');
        adminBtn.setAttribute('style', 'display: inline; margin-left: 10px; background-color: #ece5dd; border-color: #ece5dd;');
...
async function fetchMessages(id){
    ...
    messages.forEach( message => {
        ...
        const borderRadius = '10px'; // Adjust the value based on your preference
        const messageSpacing = '10px';
        const minHeight = '40px';
        const textColor = '#ece5dd'; // Set the desired text color
        const nameColor = '#3366cc';
        const youColor = '#913175';
...
if (message.UserId == userId) {
    ...
    messageBody.style.backgroundColor = '#343A40';
    messageBody.style.borderRadius = `0 ${borderRadius} ${borderRadius} ${borderRadius}`;
    messageBody.style.marginBottom = messageSpacing;
    messageBody.style.minHeight = minHeight;
    messageBody.style.display = 'flex';
    messageBody.style.alignItems = 'center';
    messageBody.style.color = textColor;
    messageBody.style.paddingLeft = leftPadding;
    messageBody.innerHTML = `<b style="color:${youColor}">You</b> : ${message.message}`;
} else {
    col.setAttribute('class', 'col-md-7');
    messageBody.setAttribute('class', 'chat-bubble chat-bubble--left');
    messageBody.style.backgroundColor = '#343A40';
    messageBody.style.borderRadius = `${borderRadius} 0 ${borderRadius} ${borderRadius}`;
    messageBody.style.marginBottom = messageSpacing;
    messageBody.style.minHeight = minHeight;
    messageBody.style.display = 'flex';
    messageBody.style.alignItems = 'center';
    messageBody.style.color = textColor;
    messageBody.style.paddingLeft = leftPadding;
    messageBody.innerHTML = `<b style="color:${nameColor}">${message.name}</b> : ${message.message}`;
}
...
async function fetchNewMessages(id, groupMessagesBox, userId){
    ...
    if(newMessages[0]){
        await newMessages.forEach(message => {
            ...
            const borderRadius = '10px'; // Adjust the value based on your preference
            const messageSpacing = '10px';
            const minHeight = '40px'; // Set a minimum height for the chat bubbles
            const nameColor = '#3366cc';
            const textColor = '#ece5dd'; // Set the desired text color
            const youColor = '#913175';
            ...
            if (message.UserId == userId) {
                ...
                messageBody.style.color = textColor;
                messageBody.style.paddingLeft = leftPadding;
                messageBody.innerHTML = `<b style="color:${youColorn}">You</b> : ${message.message}`;
            } else {
                col.setAttribute('class', 'col-md-7');
                messageBody.setAttribute('class', 'chat-bubble chat-bubble--left');
                messageBody.style.backgroundColor = '#343A40';
